...
CSV.foreach(ARGV[0], headers: true) do |row|
  ...
  docs[custodian] << Document.new(
    BatesNumber.new(beg_bates), BatesNumber.new(end_bates)
  )
end
...
CSV(STDOUT, force_quotes: true ) do |csv|
  ...
  docs.each do |custodian, documents|
    ...
    documents.each do |doc|
      if range.last_page.next == doc.first_page.to_s  # hack: compare as str
    ...
    # report ranges
    ranges << range   # add last range
    ranges.each do |range|
      csv << [custodian, range.first_page, range.last_page]
    end
